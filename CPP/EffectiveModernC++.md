...
### 条款35： 优先考虑基于任务的并发编程而非基于线程的并发编程

假设在并发状况下，我们需要执行一个函数。在标准库中我们可以选择两种方式，基于**线程**的方式和基于**任务**的方式：

```c++
int doAsyncWork();//我们需要执行的函数
std::thread t(doAsyncWork);//基于线程的方式
auto fut = std::async(doAsyncWork); //基于任务的方式，“fut”表示“future”
```

在第二种方式中，传递给`std::async`的函数对象被称之为一个**任务**(*task*).

为什么基于任务的方法更优：

+ 假如我们对待执行函数的返回值存在一定的要求，基于线程的方法对此无能为力，但是基于任务的方法可以使用`std::async`返回的`std::future`提供的`get`函数来获取返回值。
+ 如果待执行的函数发生了异常，基于线程的方式是无法进行捕获的，程序会直接终止(通过调用`std::terminate`)。但是`get`函数可以提供抛出异常的访问。

其中，最根本的原因是，基于任务的抽象层次更高，类似Windows下的完成端口，可以使我们从线程管理的细节中解放出来。在C++并发开发中有三种`thread`:

+ 硬件线程是真实执行计算的线程。现代计算机中每个CPU核心提供一个或者多个硬件线程
+ 软件线程，也称为系统线程，即内核级线程，是操作系统管理硬件线程上执行的线程。通常可以比硬件线程多。
+ `std::thread`是C++执行过程的对象，就是系统调用的封装。可以是空；可以被转移走，可以被`join`（需要运行的函数已经运行结束），也可以被`detach`（和对应的软件线程之间的连接关系被打断）

软件线程是有限的资源，如果试图创建大于系统支持的线程数量，会抛出`std::system_error`异常，即使编写的是`noexcept`版本

另一种情况是，即使没有超过软件线程的限额，仍然可能会遇到**资源超额**(*oversubscription*)的麻烦。即就绪态的线程超过了硬件线程的情况。此时操作系统的线程调度器会使用时间片轮转的方式进行处理。这就意味着存在大量的上下文切换，同一个硬件线程上，不同硬件线程上都会存在上下文切换。而且对于缓存来说，还可能出现数据污染的情况。

避免资源超额需要考虑很多问题，除了软件场景还要考虑硬件平台。个人考虑是一个很大的挑战，而`std::async`就处理了这些事。默认的启动策略`std::async`不保证会创建新的软件线程。但是，他们允许通过调度器来将特定的函数（如上述的`doAsyncWork`）运行在等待此函数结果的线程上（即在对`future`调用`get`或者`wait`的线程上），合理的调度器会在资源管理上利用这个自由度。

如果想要确保运行函数在不同的线程上进行执行，可以通过向`std::async`传递`std::launch::async`启动策略来保证函数在不同的线程上执行。

如果你直接使用`std::thread`编程，处理线程耗尽、资源超额、负责均衡问题的责任就压在了你身上，更不用说考虑不同的硬件平台了。

但是`std:thread`仍然有自己的优势：

+ 如果需要访问系统级的API，`std::thread`对象提供了`native_handle`的成员函数，而`std::future`没有这种能力。
+ 需要实现C++并发API之外的线程技术，比如C++未支持的平台的线程池。
+ 在已知的软件环境下可使用线程优化

总结就是：没本事考虑线程的资源管理，就使用标准库提供的`std::async`

### 条款36：如果异步十分重要，务必指定`std::launch::async`
